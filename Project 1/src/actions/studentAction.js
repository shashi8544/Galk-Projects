...
export const deleteStudentToShowCertificate =
	...
	(dispatch, getState, { getFirebase, getFirestore }) => {
		...
		let studentCertificateBeforeUpdate =
			getState().firebase.profile.certificate || [];
		const database = getFirestore();
...
export const updateStudentCertificate =
	(updatedInformation) =>
	(dispatch, getState, { getFirebase, getFirestore }) => {
		dispatch({
			type: actionType.CANDIDATE_UPDATE_ACTION_IN_PROGRESS,
			payload: true,
		});
		...
		let updatedStudentCertificate = studentCertificateBeforeUpdate.map((item , i) => {
			let data = { ...item };
			if (i == updatedInformation.certificateIndex) {
				...
				data.issueDate = updatedInformation.dataToUpdate.issueDate;
				data.link = updatedInformation.dataToUpdate.link;
			}
			return data;
		});
...
export const createStudentCertificate =
	(newCertificate) =>
	(dispatch, getState, { getFirebase, getFirestore }) => {
		dispatch({
			type: actionType.CANDIDATE_UPDATE_ACTION_IN_PROGRESS,
...
export const deleteStudentToShowExtraCurricular =
	...
	(dispatch, getState, { getFirebase, getFirestore }) => {
		...
		console.log(Index,"stduent index");
		let updatedStudentExtraCurricularList = studentExtraCurricularBeforeUpdate.filter(
			(item, i) => i !== Index
		);
...
export const updateStudentToShowExtraCurricular =
	...
	(dispatch, getState, { getFirebase, getFirestore }) => {
		...
		let updatedStudentExtraCurricular = studentExtraCurricularBeforeUpdate.map((item , i) => {
			let data = { ...item };
