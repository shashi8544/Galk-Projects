...
export const getApplicationPreloadData = (studentId, history, firebase) => (dispatch, {getFirebase}) => {
	dispatch({
		...
		.then((doc) => {
			if (!doc.exists){
				...
				firebase
					...
					.then(() => {
						dispatch({ type: actionTypes.SET_UNAUTHENTICATED });
						dispatch(returnErrors("You do not have permission to acces this site."));
					});
			} else {
				var data = doc.data();
				if (data) {
