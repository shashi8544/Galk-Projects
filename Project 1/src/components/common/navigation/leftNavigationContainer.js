...
import { Divider, Menu, Space, Steps } from "antd";
...
const LeftNavigationContainer = ({ activeStep, userProfile }) => {
	const jsxforProfileCompletion =
		<Steps direction="vertical" current={activeStep - 1} size="small">
			{userProfile.status != "graduate" && profileProgressSteps.map((step, i) => (
				<Step
					key={`ProgressStep_${i}`}
			...
			{userProfile.status == "graduate" && graduateProfileProgressSteps.map((step, i) => (
				<Step
					key={`ProgressStep_${i}`}
					title={`Step ${step.step}`}
					description={step.title}
	...
	const leftPanelJsx = <Space direction="vertical" style={{ width: "100%" }}>
		<LeftTop />
	...
	return (
		<div className="leftNav_container">
			{(userProfile?.profileCompletionStatus) ? leftPanelJsx : jsxforProfileCompletion}
		</div>
	);
};
