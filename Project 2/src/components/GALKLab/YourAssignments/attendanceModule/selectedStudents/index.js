...
function SelectedStudents({
	...
	const isClicked = (id) => {
		if (clickedStudent && clickedStudent.id === id) return true;
		return false;
	};
	const studentsJsx = selectedStudents?.map((elm) => (
		<div
			...
			className={`attendance-module-selected-student-card-container
                ${
									isClicked(elm.id)
										? "attendance-module-selected-student-card-container-custom"
										: ""
								}`}
		>
			...
			<div className="attendance-module-selected-student-card-student-details">
				<div className="attendance-module-selected-student-card-student-name">
					{elm.name}
				</div>
