...
import { createCandidateCertificate } from "../../../../actions/candidateActions";
...
const CreateCertificateModal = ({
	...
	const createCertificate = (formValues) => {
		...
		let { otherCertifiedSkill, otherCourseName, ...certificateToCreate } =
			formValues;

		certificateToCreate = Object.fromEntries(
			Object.entries(certificateToCreate).filter(
				([_, value]) => value !== undefined
			)
		);

		createCandidateCertificate(certificateToCreate);
	};
	...
	return (
		<Modal
			...
			footer={null}
		>
			<Form layout="vertical" form={form} onFinish={createCertificate}>
				<Form.Item
					label="Title"
					name="title"
					rules={[
