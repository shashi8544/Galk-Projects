...
const EditJobModal = ({
	...
	const updateJob = (formValues) => {
		formValues.startDate = formValues.startDate
			? moment(formValues.startDate).format("LL")
			: null;
		formValues.endDate = formValues.endDate
			? moment(formValues.endDate).format("LL")
			: null;
		let dataToUpdate = {
			...formValues,
			// id: jobDetails.id,
...
export default connect(mapStateToProps, { updateStudentJob })(EditJobModal);
